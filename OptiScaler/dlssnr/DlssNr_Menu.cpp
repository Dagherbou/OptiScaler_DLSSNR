...
void RenderMenu(Config* config, float menuResScale)
{
    ...
    if (auto ch = ScopedCollapsingHeader("DLSS Neural Rendering"); ch.IsHeaderOpen())
    {
...

        {
            ...
            if (ImGui::Checkbox("Take white point from the game", canEnable ? &shown : &disabledUnchecked) && canEnable)
            {
                config->DlssNrWhitePointFromExposure = shown;
            }
            ImGui::EndDisabled();

            std::string boxHelp = "When a game hands DLSS a 1\xC3\x97"
                                  "1 exposure texture, that texture is the\n"
            ...
            case DlssNr::GameExposureWait::NativeVulkan:
                ImGui::TextDisabled(
                    "This path cannot sample the game\xE2\x80\x99s exposure. Paper white below is in use.");
                break;
            ...
            if (ImGui::SliderFloat("Paper white", &wpScale, 0.25f, 240.0f, effective ? "%.2f\xC3\x97" : "%.2f",
            ...
            if (canEnable)
            {
                ...
                {
                    ImGui::TextColored(live, "Exposure \xE2\x80\x94 \xC2\xB7 pre %.2f \xC2\xB7 scale %.2f", readout.p,
                                       readout.s);
                    if (effective)
